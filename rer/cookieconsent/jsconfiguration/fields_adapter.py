...
from rer.cookieconsent.utils import get_url_to_dashboard, get_url_to_path
...
class CookieBannerSettingsAdapter(Object):
    ...
    def data(self, record):
        ...
        privacy_link_url = get_url_to_path(result['privacy_link_url'])
        ...
        new_text = pattern_privacy_link.sub(
            URL_MODEL.format(privacy_link_url, privacy_link_text, r'\2'), new_text
        )
        ...
        new_text = pattern_dashboard_link.sub(
            URL_MODEL.format(dashboard_link_url, dashboard_link_text, r'\2'), new_text
        )
        new_text = pattern_dashboard_link_url.sub(dashboard_link_url + r'\2', new_text)
        new_text = pattern_dashboard_link_text.sub(
            dashboard_link_text + r'\2', new_text
        )
